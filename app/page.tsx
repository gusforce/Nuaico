...
import { featuredNews, getTrendingArticles, ALL_ARTICLES, SECTORS } from "@/lib/data"
...
export default function Home() {
  ...
  if (isCorp) {
    ...
    const sortedByImpact = [...ALL_ARTICLES].sort((a, b) => (b.impactScore ?? 0) - (a.impactScore ?? 0))
    const priorityBriefing = sortedByImpact.slice(0, 3)

    // Calculate per-sector stats for the Sector Intelligence Grid
    const sectorStats = SECTORS.map(sector => {
      const articles = ALL_ARTICLES.filter(a => a.sector === sector)
      const topArticle = [...articles].sort((a, b) => (b.impactScore ?? 0) - (a.impactScore ?? 0))[0]
      const highestImpact = topArticle?.impactScore ?? 0
      return { sector, count: articles.length, topArticle, highestImpact }
    })
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-3 text-sm text-slate-500">
                <span className="inline-flex items-center gap-1.5 bg-slate-100 px-3 py-1 rounded-full font-medium">
                  ...
                  {ALL_ARTICLES.length} articles analyzed
                </span>
              </div>
            </div>
          </div>
  ...
  const filteredArticles = selectedSector === 'All'
    ? ALL_ARTICLES
    : ALL_ARTICLES.filter(a => a.sector === selectedSector)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <section className="pb-8">
          {filteredArticles.length === 0 && (
            <p className="text-center text-white/50 py-16">No articles found for this sector.</p>
          )}
